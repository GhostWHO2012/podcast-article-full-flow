...
`youtube-to-weixin-article` 是一个把访谈内容转成深度长文和公众号长图的 Agent Skill。它面向内容团队，把 YouTube 视频、播客访谈、线上分享、字幕、截图和视频链接整理成中文公众号深度长文，并输出可检查的公众号竖向长图。当前版本已将独立双盲评估中反复出现的事实核验、证据密度、开头信息密度和手机端截图可读性要求写入用户版 Skill。
...
| 证据核验 | 对价格、模型版本、测试条件、样本量和比较结论逐项核对，无法核验的内容标记为推测或删除。 |
...
晚点原内容仅作为学习和对照基准，用来总结标题命名、段落节奏、截图位置、来源展示和长图排版规则。仓库内只保留已整理的对照入口和授权后的原长图资产，不展示本地文件夹目录。
...
| 开头 | 先给三条有来源的金句，再用具体场景或冲突导入，并在前 100 到 150 字建立核心问题、主要结论和读者收益。 |
| 篇幅 | 常规成稿控制在约 4000 到 5500 中文字，即使字幕很长也要强压缩。 |
| 细节 | 保留卡住、不安、失败、等待、误判、成本和真实用户反馈等能支撑判断的故事细节，压缩重复观点。 |
| 截图 | 优先选择数据、演示、工作流、生活/工作现场、动作手势帧，再考虑普通人物访谈帧；交付前按手机宽度检查可读性，并用图注指出证据。 |
